...
private:
  ...
  void parse_number(value& v, int ch)
  {
    ...
    if ((frac_part == 0) && (exp_part == 0)) {
      if (negative) {
		  value::integer_type tmp = static_cast<value::integer_type>(int_part);
        const auto integer_value = static_cast<value::integer_type>(-tmp);
        if (static_cast<decltype(int_part)>(integer_value) == -tmp) {
          v = integer_value;
          return;
        }
      } else {
